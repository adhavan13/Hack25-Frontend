  ...
  const filterGrievances = (grievancesList) => {
    return grievancesList.filter(grievance => {
      const matchesSearch = !searchTerm || 
        grievance.grievance_title?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        grievance.short_description?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        grievance.long_description?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        ...
        grievance.project_service_name?.toLowerCase().includes(searchTerm.toLowerCase());
      ...
      const matchesCategory = selectedCategory === 'All' || 
        ...
        grievance.category?.toLowerCase().includes(selectedCategory.toLowerCase());
      
      const matchesLocation = !selectedLocation || 
        !grievance.location || // Include grievances without location
        grievance.location?.toLowerCase().includes(selectedLocation.toLowerCase());
  ...
  useEffect(() => {
    const fetchAllGrievances = async () => {
      ...
      try {
        ...
        console.log(`Fetched total of ${allFetchedGrievances.length} grievances from database`);
  ...
  const handlePostSuccess = () => {
    ...
    const fetchAllGrievances = async () => {
      ...
      try {
        ...
        console.log(`Refreshed total of ${allFetchedGrievances.length} grievances from database`);
        setAllGrievances(allFetchedGrievances);
        const filtered = filterGrievances(allFetchedGrievances);
        setGrievances(filtered);
      } catch (err) {
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                ...
                {selectedLocation && (
                  <span className="inline-flex items-center gap-1 px-2 py-1 bg-orange-100 text-orange-800 text-xs font-medium rounded-full">
                    ...
                    <button onClick={() => setSelectedLocation('')} className="hover:bg-orange-200 rounded-full p-0.5">
                      <X size={12} />
                    </button>
                  </span>
                )}
                ...
                {grievances.map((grievance) => (
                  ...
                  >
                    <GrievanceCard
                      ...
                      description={
                        grievance.long_description || grievance.short_description
                      }
